...
def save_phase_image(data, filename):
    """Save the unwrapped phase data as an image using OpenCV."""
    # Normalize the data to the range 0-255 for 8-bit image representation
    ...
    plt.imsave(filename, data_normalized,colormap='gray')
...
def main():
    ...
    cubic_phase_plate1 = create_cubic_phase_plate(n, px, c0, wav, r_air, r_mat, delta1, delta2)

    phase1 = np.angle(cubic_phase_plate1) + np.pi
    # phase2 = np.angle(cubic_phase_plate2)

    unwrapped_phase1 = unwrap_phase(phase1) * 255 / (2 * np.pi)  # convert to mm
    # unwrapped_phase2 = unwrap_phase(phase2) * wav / (2 * np.pi) * 1e+3  # convert to mm

    save_phase_image(unwrapped_phase1, 'CBP1sag.png')
